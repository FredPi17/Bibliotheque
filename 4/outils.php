  ...
  */
  {
	...
	*/
	{
		echo'<footer> <a href="index.php"Accueil</a><a href="listelivre.php"Liste</a><a href="about.php"A propos</a> <p>Site réalisé par les B1 EPSI Grenoble, 2016-2017,<a href="#"CGU</a></p>
		</footer>
		</body></html>';
	}
...

  {
	...
	*/
		...
		$bdd->exec("Set character set utf8");
				...
				/* -------------------- FONCTION MESSAGES() ------------------ */
				function Messages()       /*je cree ma fonction message*/

			  {
			    $sortie = "";           /*sortie de la fonction*/

			    if (!empty($_SESSION['msgErreur']))   /* si l'utilisateur ne saisi rien je renvoie une erreur*/

			     {
			      $sortie .= '<div class="erreur">' . $_SESSION['msgErreur'] . '</div>'; /* j'affiche le message d'erreur*/
			    }

			    if (!empty($_SESSION['msgOk']))/*si l'utilisateur a effectuer une saisie je renvoie ok */
			     {
			      $sortie .= '<div class="info">' . $_SESSION['msgOk'] . '</div>';     /*j'affiche msg ok*/
			    }

			    $_SESSION['msgErreur'] = '';
			    $_SESSION['msgOk'] = '';

			    return $sortie; /* Je renvoie la variable sortie  */
			  }

			  /* POUR TESTER LA FONCTION MESSAGES */
			  /* session_start();
